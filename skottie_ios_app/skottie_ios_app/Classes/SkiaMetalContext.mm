...
- (void)drawRect:(CGRect)rect {
	...
	if (!viewController || ![[self currentDrawable] texture] || !fDContext || !fQueue) {
		return;
	}
...
//	fDContext = GrDirectContext::MakeMetal((__bridge_retained void *)[self metalDevice],
//	                                       (__bridge_retained void *)[self metalQueue],
//	                                       GrContextOptions());

	fDContext = SkMetalDeviceToGrContext(self.metalDevice, self.metalQueue);
